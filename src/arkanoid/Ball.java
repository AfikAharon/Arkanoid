...
public class Ball implements Sprite {
    ...
    public void moveOneStep() {
        ...
        double movementY;
        /**
         * conditions if the the current location + next step smaller than current location + the ball radius,
         * to ensure that the ball not digress to the borders.
         *
         */
        if (this.vel.getDx() < this.getSize() && this.vel.getDx() > 0) {
            movementX = this.p.getX() + this.getSize();
        } else if (this.vel.getDx() > (-1) * this.getSize() && this.vel.getDx() < 0) {
            movementX = this.p.getX() - this.getSize();
        } else {
            movementX = this.p.getX() + this.vel.getDx();
        }
        if (this.vel.getDy() < this.getSize() && this.vel.getDy() > 0) {
            movementY = this.p.getY() + this.getSize();
        } else if (this.vel.getDy() > (-1) * this.getSize() && this.vel.getDy() < 0) {
            movementY = this.p.getY() - this.getSize();
        } else {
            movementY = this.p.getY() + this.vel.getDy();
        }
